...
class DNSClient: NSObject, GCDAsyncUdpSocketDelegate {
    ...
    ///
    fileprivate let delegateQueue = DispatchQueue(label: "at.alladin.dns.client", attributes: DispatchQueue.Attributes.concurrent)
...
// MARK: class query methods
    ...
    class func query(_ qname: String, recordType: UInt16, success successCallback: @escaping DNSQuerySuccessCallback, failure failureCallback: @escaping DNSQueryFailureCallback) {
        ...
        let dnsClient: DNSClient = DNSClient()
        dnsClient.query(qname, recordTypeInt: recordType, success: { responseString in
...
// MARK: query methods
    ...
    ///
    func query(_ qname: String, recordTypeInt: UInt16, success successCallback: @escaping DNSQuerySuccessCallback, failure failureCallback: @escaping DNSQueryFailureCallback) {
        queryNameserver(dnsServer, serverPort: dnsPort, forName: qname, recordTypeInt: recordTypeInt, success: successCallback, failure: failureCallback)
    }
...
// MARK: other
    ...
    func buildQueryData(_ qname: String, recordType: UInt16) -> Data {
        ...
        var requestDNSHeader = DNSHeader()
        ...
        let requestHeaderData = Data(bytes: &requestDNSHeader, count: MemoryLayout<DNSHeader>.size) // TODO: is this correct?
        //let requestHeaderData = Data(bytes: UnsafePointer<UInt8>(&requestDNSHeader), count: sizeof(DNSHeader))
        ...
        var requestDNSQuestion = DNSQuestion()
        ...
        let request2BodyData = Data(bytes: &requestDNSQuestion, count: MemoryLayout<DNSQuestion>.size) // TODO: is this correct?
        //let request2BodyData = Data(bytes: UnsafePointer<UInt8>(&requestDNSQuestion), count: sizeof(DNSQuestion))
    ...
    @objc func udpSocket(_ sock: GCDAsyncUdpSocket, didReceive data: Data, fromAddress address: Data, withFilterContext filterContext: Any?) {
        ...
        DispatchQueue.global(qos: DispatchQoS.QoSClass.default).sync { // needed because otherwise println's would merge strangely, TODO: remove this later

            var dnsRecordClass: DNSRecordClass!

//            println("didReceiveData: \(data)")

            let subdata = data.subdata(in: 0..<MemoryLayout<DNSHeader>.size) // TODO: is this correct?
            //let subdata = data.subdata(in: NSRange(location: 0, length: MemoryLayout<DNSHeader>.size))
...
//            println(dnsHeader)
            ...
            let subdata2 = data.subdata(in: MemoryLayout<DNSHeader>.size..<data.count) // TODO: is this correct?
...
//            println("qname \(qname)")
            ...
            let zeroByteArray = [UInt8](repeating: 0, count: MemoryLayout<UInt8>.size) // TODO: is this correct?
            //var zeroByte = [UInt8](repeating: 0, count: 1)

            let zeroByteData = Data(bytes: zeroByteArray) // TODO: is this correct?
            //let zeroByteData = Data(bytes: UnsafePointer<UInt8>(&zeroByte), count: MemoryLayout<UInt8>.size)

            let d_new_range = subdata2.range(of: zeroByteData, options: [], in: 0..<subdata2.count)! // TODO: is this correct? // TODO: !
            //let d_new_range = subdata2.range(of: zeroByteData, options: [], in: NSRange(location: 0, length: subdata2.count))

//            println("d_new_range: \(d_new_range)")

            let d: Data = subdata2.subdata(in: d_new_range.lowerBound + 1..<subdata2.count) // TODO: is this correct?
            //let d: Data = subdata2.subdata(with: NSRange(location: d_new_range.location + 1, length: subdata2.count - (d_new_range.location + 1)))
...
//            println("d: \(d)")
            ...
            if dnsHeader.anCount > 0 {
                ...
                let dd = d.subdata(in: MemoryLayout<DNSQuestion>.size..<d.count) // TODO: is this correct?
                //let dd = d.subdata(in: NSRange(location: MemoryLayout<DNSQuestion>.size, length: d.count - MemoryLayout<DNSQuestion>.size))
...
//                println("dd: \(dd)")
                ...
                let ddr = dd.subdata(in: 0..<MemoryLayout<DNSResourceRecord>.size) // TODO: is this correct?
...
//                println("ddr: \(ddr)")
                ...
                let rdata = dd.subdata(in: 2+2+2+4+2..<2+2+2+4+2 + Int(dnsResourceRecord.dataLength)) // TODO: is this correct
                ...
                case kDNSServiceType_MX: // mx
                    ...
                    let exchangeData = rdata.subdata(in: MemoryLayout<UInt16>.size..<rdata.count) // TODO: is this correct?
...
//                )
    ...
    ///
    @objc func udpSocketDidClose(_ sock: GCDAsyncUdpSocket, withError error: Error?) { // crashes if NSError is used without questionmark
    ...
    fileprivate func parseNameFromData(_ subData: Data, data: Data, nameOffset: UInt) -> String {
        ...
        while localOffset < /*data*/subData.count {
            ...
            if firstByteOfBlock >= 192 {
                ...
                let rangeData = subData.subdata(in: localOffset..<Int(firstByteOfBlock) + localOffset) // TODO: is this correct?
                //let rangeData = subData.subdata(in: NSRange(location: localOffset, length: Int(firstByteOfBlock)))
                let partStr = String(data: rangeData, encoding: String.Encoding.ascii)!

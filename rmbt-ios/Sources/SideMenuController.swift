...
class SideMenuController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if NAVIGATION_USE_TINT_COLOR {
            tintButton(infoButton)
            tintButton(settingsButton)
        }
    }

    ///
    fileprivate func tintButton(_ button: UIButton?) {
        button?.tintColor = RMBT_TINT_COLOR

        if let imageView = button?.imageView, let image = imageView.image {
            //imageView.tintColor = RMBT_TINT_COLOR

...
extension UITableViewCell: RMBTHeight {
    ...
    class func rmbtApproximateOptimalHeightForText(_ text: String?, detailText: String?) -> CGFloat {
        ...
        if let t = text {
            textSize = t.size(attributes: ["NSFontAttributeName": _uitableviewcell_cellTextFont])
        }

        var detailTextSize = CGSize(width: 0, height: 0) // default
        if let t = detailText {
            detailTextSize = t.size(attributes: ["NSFontAttributeName": _uitableviewcell_cellDetailTextFont])
        }

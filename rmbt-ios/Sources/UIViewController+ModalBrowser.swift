...
extension UIViewController: ModalBrowser {
    ...
    ///
    func presentModalBrowserWithURLString(_ url: String) {
        if let webViewController = KINWebBrowserViewController.navigationControllerWithWebBrowser() {
            ...
            webViewController.navigationBar.items?.forEach({ (item) in
                item.leftBarButtonItems?.forEach({ (bi) in
                    bi.setTitleTextAttributes([NSFontAttributeName: font], for: .normal)
                })
                ...
                item.backBarButtonItem?.setTitleTextAttributes([NSFontAttributeName: font], for: .normal)
            })
